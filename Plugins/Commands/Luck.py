...
bad_things = (
'造世吞（直接放飞', '修机器（一修就炸', '挖矿（只挖到原石', '造建筑（啥都没有', '钓鱼（全部是垃圾', '刷附魔（刷的垃圾','脚皮痛(扣到jpt')
good_things = (
'造世吞（完美运行', '修机器（一修就好', '挖矿（挖到十钻石', '造建筑（要啥都有', '钓鱼（钓到把神弓', '刷附魔（一发就中','吃到麦当劳(跳梁小丑')
...
def luck_handler(event: GroupMessageEvent):
    ...
    if luck_point > 90:
        ...
        tips = '呜……'
    yield F'你今天的脚品为 {luck_point}，{tips}'
